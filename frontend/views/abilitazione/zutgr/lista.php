...
<div class="quiz-index">
    ...
    <p style="margin-bottom:0px; margin-top:5px">
		<?php echo frontend\controllers\BaseController::linkwin('Aggiungi|fa-plus', 'abilitazione/zutgr/create', [], 'Inserisci un nuovo elemento','document.location.reload(false)',['windowtitle'=>'Inserisci un elemento','windowwidth'=>'700']); ?>
		<!--a class="btn btn-success" onclick="apriForm(this, '/index.php?r=quiz/create')" href="javascript:void(0)" title="Update" aria-label="Update" data-pjax="0"><span class="fas fa-plus" aria-hidden="true"></span>Create zUtGr</a-->	
    </p>
    ...
    <?php
		...
		foreach ($models as $riga) {?>
			<tr id='RigazUtGr_<?=$pos?>' chiave="<?=$riga->idutgr?>">
				<td>
					<?php echo frontend\controllers\BaseController::linkwin('Edit|fa-edit', 'abilitazione/zutgr/view', ['idutgr'=>$riga->idutgr], 'Apri per modifica','document.location.reload(false)',['windowtitle'=>'Inserisci i parametri','windowwidth'=>'700']); ?>
				</td>   
				<td><?= $riga->idgruppo?> <?=$riga->zgruppo->nomegruppo ?></td>
				<td><?= $riga->id?> <?=$riga->user->soggetto->NomeSoggetto?></td>
				<td><?= $riga->idutgr ?></td>
